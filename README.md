...
<a name="последние-изменения"></a>
 В последней версии бота главным изменением является алгоритм его работы. Теперь бот взаимодействует с пользователем посредством кнопок. Реализовано новое поле `events`, в котором хранятся события, загруженные пользователем. Так же реализован класс `User`, позволяющий сохранять события и расписание пользователя при непрерывной работе сервера. Появились методы `edit timetable` и `edit events`. Теперь информация о пользователях хранится в глобальном словаре, элементаи которого являются объекта класса `User`, а тэгами их `telegram id`.
В новой версии бот загружает расписание с сайта МГТУ, посредством работы с модулями `requests`, `bs4`.
