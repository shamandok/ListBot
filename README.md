...
<a name="последние-изменения"></a>
 В последней версии бота главным изменением является __алгоритм его работы__. Теперь: 
* Бот взаимодействует с пользователем посредством *кнопок* 
* Реализовано новое поле `events`, в котором хранятся события, загруженные пользователем
* Реализован класс `User`, позволяющий сохранять события и расписание пользователя при непрерывной работе сервера
* Появились методы `edit timetable` и `edit events`
* Теперь информация о пользователях хранится в глобальном словаре, элементаи которого являются объекта класса `User`, а тэгами их `telegram id`.
* В новой версии бот загружает расписание с сайта __МГТУ__, посредством работы с модулями `requests`, `bs4`.
* Новая версия бота уже выгружена на `PyPI`, с добавленным описанием в формате `README.rst`.
...
3. С помощью `git`
```
$ git clone https://github.com/vladislavburch/listbot.git
$ cd listbot
